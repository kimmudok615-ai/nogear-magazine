...
def extract_hero_number(text: str) -> dict:
    ...
    # 패턴: 93%, 9배, 40%, $11,000, 2,986%, 1만 달러, 3억 등
    # (정규식, 단위, 우선순위, 표시템플릿) — 표시템플릿이 None이면 group(0) 사용
    patterns = [
        (r'(\d+(?:,\d+)*(?:\.\d+)?)%', '%', 1, None),        # 93%, 0.24%
        (r'(\d+)배', '배', 2, None),                          # 9배
        (r'\$(\d+(?:,\d+)*)', '$', 0, None),                  # $11,000
        # 한국식 만/억 단위 (달러·원 모두) — '1만 달러' → '1만$'
        (r'(\d+(?:,\d+)*)\s*만\s*달러', '$', 0, '{n}만$'),     # 1만 달러
        (r'(\d+(?:,\d+)*)\s*억\s*달러', '$', 0, '{n}억$'),     # 1억 달러
        (r'(\d+(?:,\d+)*)\s*억', '억', 0, '{n}억'),           # 3억
        (r'(\d+(?:,\d+)*)\s*만\s*원', '원', 0, '{n}만원'),     # 500만 원
        (r'(\d+(?:,\d+)*)\s*만(?![가-힣])', '만', 1, '{n}만'), # 1만 (뒤에 한글 없을 때)
        (r'(\d+(?:,\d+)*)\s*(?:원|만원)', '원', 0, None),
    ...
    for pattern, unit, priority, template in patterns:
        ...
        if m:
            full = template.format(n=m.group(1)) if template else m.group(0)
            return {
                "number": m.group(1),
                "unit": unit,
                "priority": priority,
